...
BOOLEAN BlIsValidGUIDPartitionTableHelper(__in PVOID Buffer,__in ULONGLONG LBA,__in PULONG DiskId,__in PGPT_DISK_READ_CALLBACK Callback)
{
	...
	*Add2Ptr(Buffer,0x10,PULONG)						= SavedChecksum;
	...
	// check MyLBA
	//
	//ULONGLONG MyLBA										= *Add2Ptr(Buffer,0x18,PULONGLONG);
	//if(MyLBA != LBA)
	//	return FALSE;
...
ARC_STATUS HardDiskPartitionOpen(__in ULONG FileId,__in ULONG DiskId,__in UCHAR PartitionNumber)
{
	...
	do
	{
		ULARGE_INTEGER SeekPosition;
		SeekPosition.QuadPart							= static_cast<ULONGLONG>(PartitionOffset) << SECTOR_SHIFT;
		...
		if(Status == ESUCCESS)
		{
			...
			for(ULONG i = 0; i < 4; i ++)
			{
				...
				//	0x0f,xint13
				//
				UCHAR Type								= *Add2Ptr(DataBuffer,i * 0x10 + 0x1be + 0x04,PUCHAR);
				...
				if(PartitionCount == PartitionNumber)
				{
					...
					Context->PartitionLength.QuadPart	= static_cast<ULONGLONG>(PartitionLength) << SECTOR_SHIFT;
					Context->StartingSector.QuadPart	= PartitionOffset + StartingSector;
...
{
	ULARGE_INTEGER SeekPosition;
	SeekPosition.QuadPart								= LBA << SECTOR_SHIFT;
...
ARC_STATUS BlOpenGPTDiskPartition(__in ULONG FileId,__in ULONG DiskId,__in UCHAR PartitionNumber)
{
	...
	SeekPosition.QuadPart								= PartitionEntryLBA << SECTOR_SHIFT;
	Status												= BlFileTable[DiskId].DeviceEntryTable->Seek(DiskId,&SeekPosition,SeekAbsolute);
	if(Status != ESUCCESS)
		return Status;
	...
	Context->EndingSector.QuadPart						= *Add2Ptr(PartitionTable,0x28,PULONGLONG);
	Context->StartingSector.QuadPart					= *Add2Ptr(PartitionTable,0x20,PULONGLONG);
	Context->PartitionLength.QuadPart					= (Context->EndingSector.QuadPart - Context->StartingSector.QuadPart) << SECTOR_SHIFT;
	BlFileTable[FileId].Position.QuadPart				= 0;

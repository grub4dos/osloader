...
// seek image cache
//
ARC_STATUS BlImageSeek(__in PIMAGE_CACHE ImageCache,__in ULONG FileId,__inout PULARGE_INTEGER SeekOffset,__in SEEK_MODE SeekMode)
{
	//
	// too big
	//
	...
	__try
	{
		...
		for(ULONG i = 0; i < NumberOfSections; i ++,SectionHeader ++)
		{
			...
			if(SizeOfRawData < VirtualSize)
				RtlZeroMemory(Add2Ptr(ImageBase,SizeOfRawData + SectionHeader->VirtualAddress,PVOID),VirtualSize - SizeOfRawData);
		}

...
ARC_STATUS BiosDiskOpen(__in ULONG DriveId,__in OPEN_MODE OpenMode,__out PULONG FileId)
{
	...
	else
	{
		...
		do
		{
			...
			NumberCylinders								= static_cast<USHORT>(((Result >> 24) + ((Result >> 14) & 0x300)) + 1);
...
{
	ULARGE_INTEGER PhysicalOffset;
	PhysicalOffset.QuadPart								= BlFileTable[FileId].Position.QuadPart + (BlFileTable[FileId].u.PartitionContext.StartingSector.QuadPart << SECTOR_SHIFT);
...
// seek bios partition
//
ARC_STATUS BiosPartitionSeek(__in ULONG FileId,__in PULARGE_INTEGER Offset,__in SEEK_MODE SeekMode)
{
	switch(SeekMode)
	{
	case SeekAbsolute:
...
ARC_STATUS BiosPartitionGetFileInfo(__in ULONG FileId,__out PFILE_INFORMATION Finfo)
{
	...
	Finfo->StartingAddress.QuadPart						= Context->StartingSector.QuadPart;

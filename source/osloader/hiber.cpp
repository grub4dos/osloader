...
{
	ULARGE_INTEGER LargeOffset;
	LargeOffset.QuadPart								= static<ULONGLONG>(Offset) << PAGE_SHIFT;
...
BOOLEAN HbReadNextCompressedPages(__in ULONG ReadBytesCount,__in PHIBER_COMPRESSED_READ_STATE ReadState)
{
	...
	if(ReadState->SeekFileBeforeRead)
	{
		...
		Offset.QuadPart									= static_cast<ULONGLONG>(ReadState->OffsetInPage) << PAGE_SHIFT;

...
// cache read
//
ARC_STATUS BlDiskCacheRead(__in ULONG DeviceId,__in PULARGE_INTEGER Offset,__in PVOID Buffer,__in ULONG Count,__in PULONG ReadCount,__in BOOLEAN SaveDataToCache)
{
	*ReadCount											= 0;
			...
			for(ULONG i = 0; i < RunsCount; i ++)
			{
				...
				if(SaveDataToCache)
				{
					...
					while(DstBuffer < EndOfDstBuffer)
					{
						...
						// read disk data
						//
						Status							= ArcSeek(DeviceId,reinterpret_cast<PULARGE_INTEGER>(&NewRange->StartOffset),SeekAbsolute);
						if(Status != ESUCCESS)
							__leave;
				...
				else
				{
					...
					SeekOffset.QuadPart					= static_cast<ULONGLONG>(StartOffset);
					PVOID DstBuffer						= Add2Ptr(Buffer,static_cast<ULONG>(StartOffset - Offset->QuadPart),PVOID);
...
// cache write
//
ARC_STATUS BlDiskCacheWrite(__in ULONG DeviceId,__in PULARGE_INTEGER Offset,__in PVOID Buffer,__in ULONG Count,__in PULONG WritenCount)
{
	PDISK_CACHE_ENTRY Entry								= BlDiskCacheFindCacheForDevice(DeviceId);
	if(Entry)
	{

								   ...
								   __inout PBOOLEAN UseLastKnownGood,__out PBOOLEAN LoadSacDriver,__out PCHAR BadFile)
{
	...
	__try
	{
		...
		//
		BOOLEAN AlternateUsed							= FALSE;
		BOOLEAN RestartSetup							= FALSE;
		Status											= BlLoadAndInitSystemHive(DeviceId,DeviceName,LocalPath,"system",FALSE,&RestartSetup,&AlternateUsed);
		...
		if(Status != ESUCCESS)
		{
			...
			//
			if(!AlternateUsed)
				Status									= BlLoadAndInitSystemHive(DeviceId,DeviceName,LocalPath,"system.alt",TRUE,&RestartSetup,&AlternateUsed);
		...
		//
		// if we are restarting setup,try to change to system.sav
		//
		if(RestartSetup)
			Status										= BlLoadAndInitSystemHive(DeviceId,DeviceName,LocalPath,"system.sav",TRUE,&RestartSetup,&AlternateUsed);
